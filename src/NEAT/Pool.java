...
public class Pool {
            ...
            for (Genome g : s.getGenomes()) {
                ...
                for (int i = 0; i < 2; i++)
                    for (int j = 0; j < 2; j++) {
                        ...
                        int expected = i ^ j;
                        // System.out.println("Inputs are " + inputs[0] +" " + inputs[1] + " output " +
                        // output[0] + " Answer : " + (i ^ j));
                        // if (output[0] == (i ^ j))
    ...
    private void rankGlobally() { // set fitness to rank
        ...
        // System.out.println("TopFitness : "+
        // allGenome.get(allGenome.size()-1).getFitness());
        for (int i = 0; i < allGenome.size(); i++) {
    ...
    public ArrayList<Genome> breedNewGeneration() {
        ...
        float carryOver = 0;
        for (Species s : species) {
            float fchild = NEAT_Config.POPULATION * (s.getTotalAdjustedFitness() / globalAdjustedFitness);// - 1; //
                                                                                                          // reconsider
